/* BlueBlock1750v4.ino
 ...
 The autoconnect is turned on and HFP is turned off.
...
int counter = 0; //counter for BC127's status so the RedBoard checks state after a certain period of time
...
void loop() {
  ...
  if(swPort.available() > 0) {
    //c = swPort.read();
    buffer_temp.concat(char(swPort.read()));
  }
  delay(300);
  if (swPort.overflow()) {
...
STATE CONNECTED
 ...
 ----------AUTCOCONNECT BUT NOT DISCOVERABLE----------
 STATE CONNECTABLE
